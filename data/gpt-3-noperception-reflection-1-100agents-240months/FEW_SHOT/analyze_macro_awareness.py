...
# 正确提取：action是字典 {'SimpleLabor': 0/1, 'SimpleConsumption': 0-50}
...
for t, action in enumerate(actions):
    for i in range(n_agents):
        ...
        if agent_id in action:
            ...
            consumption_prop = consumption_idx * 0.02  # 转换为比例
...
print(f"✅ 提取完成")
print(f"  Agent 0的决策数: {len(agent_work[0])}")
print(f"  Agent 0前5个work决策: {agent_work[0][:5]}")
print(f"  Agent 0前5个consumption决策: {[f'{c:.2f}' for c in agent_consumption[0][:5]]}")
...
print(f"\n✅ 数据验证:")
print(f"  总就业决策次数: {total_work_decisions}")
print(f"  平均每月就业人数: {total_work_decisions/len(actions):.1f}")
print(f"  总消费金额: {total_consumption:.0f}")
...
print(f"✅ 宏观指标计算完成")
print(f"  失业率数据点: {len(unemployment_rate)}")
print(f"  平均消费数据点: {len(avg_consumption_prop)}")
print(f"  GDP增长率数据点: {len(gdp_growth)}")
...
print(f"\n失业率:")
print(f"  均值: {np.mean(unemployment_rate)*100:.2f}%")
print(f"  中位数: {np.median(unemployment_rate)*100:.2f}%")
print(f"  范围: [{np.min(unemployment_rate)*100:.2f}%, {np.max(unemployment_rate)*100:.2f}%]")

print(f"\n平均消费倾向:")
print(f"  均值: {np.mean(avg_consumption_prop):.3f}")
print(f"  中位数: {np.median(avg_consumption_prop):.3f}")
print(f"  范围: [{np.min(avg_consumption_prop):.3f}, {np.max(avg_consumption_prop):.3f}]")

print(f"\nGDP增长率:")
print(f"  均值: {np.mean(gdp_growth)*100:.2f}%")
print(f"  中位数: {np.median(gdp_growth)*100:.2f}%")
print(f"  标准差: {np.std(gdp_growth)*100:.2f}%")
print(f"  范围: [{np.min(gdp_growth)*100:.2f}%, {np.max(gdp_growth)*100:.2f}%]")
...
for state, count in state_counts.items():
    print(f"  {state:10s}: {count:3d} 个月 ({count/len(df_analysis)*100:.1f}%)")
...
if len(df_corr) > 10:
    ...
    print(f"\n消费变化 vs GDP增长变化的相关系数: {correlation:.3f}")
    
    if correlation > 0.3:
        print("\n  ⚠️ 正相关！Agent倾向于顺周期消费（加剧波动）")
        print("     解释：")
        print("     - 经济好时，agent增加消费 → 可能导致过热")
        print("     - 经济差时，agent减少消费 → 加剧衰退")
        print("     - Agent缺乏'逆周期调节'能力")
    elif correlation < -0.3:
        print("\n  ✅ 负相关！Agent倾向于逆周期消费（稳定经济）")
        print("     解释：")
        print("     - 经济好时，agent减少消费（储蓄）")
        print("     - 经济差时，agent增加消费（刺激）")
    else:
        print("\n  ➡️ 弱相关。Agent决策与宏观周期关系不明显")
        print("     解释：")
        print("     - Agent似乎不关注宏观经济状态")
        print("     - 决策主要基于个人财富/收入")
...
print("\n1. 宏观意识缺失:")
print(f"   - 消费-GDP相关性: {correlation:.3f}")
if abs(correlation) < 0.2:
    print("   - ⚠️ Agent决策与宏观周期几乎无关")
    print("   - Agent不知道经济在衰退还是繁荣")

print("\n2. 经济周期:")
衰退比例 = (df_analysis['economic_state'] == 'Recession').sum() / len(df_analysis)
繁荣比例 = (df_analysis['economic_state'] == 'Boom').sum() / len(df_analysis)
print(f"   - 衰退期占比: {衰退比例*100:.1f}%")
print(f"   - 繁荣期占比: {繁荣比例*100:.1f}%")

print("\n3. 为什么符号一致性只有31.58%:")
print("   - Agent只能看到自己的微观状态（收入、财富）")
print("   - 无法获取宏观信号（整体失业率、通胀、GDP）")
print("   - 缺乏'预见性'，只能'反应性'决策")
print("   - 结果：无法预测经济增长/衰退方向")

print("\n4. 改进建议:")
print("   a) 在prompt中注入宏观信号")
print("   b) 提供'经济学家建议'（基于宏观状态）")
print("   c) 让agent知道'现在是衰退/繁荣期'")
...
with open(result_path, 'w') as f:
    ...
    f.write(f"recession_ratio,{衰退比例}\n")
    f.write(f"boom_ratio,{繁荣比例}\n")
