...
# ========== 1. Load data ==========
data_path = r'/workspace/QWEN2.5-7b/data/gpt-3-noperception-reflection-1-100agents-240months'
...
# ========== Helper functions ==========
A = 1  # productivity
num_labor_hours = 168  # monthly work hours
...
def calculate_monthly_gdp(t, states, actions, prices):
    """Calculate monthly GDP = S × P"""
    monthly_supply = 0
    for agent_id, action in actions[t].items():
        if agent_id == 'p':
            continue
        
        # Extract labor decision
...
# ========== 3. Compute macro metrics (monthly unemployment + yearly GDP/inflation) ==========
print("\nStep 1: Filtering months with good macro performance...")

max_t = min(len(states), len(periodic_tax), len(actions), len(prices))
macro_good_months = []  # store month indices, not years
...
for year in range(2, 21):
    ...
    # Yearly macro constraints
    gdp_good = (-2.0 <= gdp_growth <= 6.73)
    inflation_good = (-0.356 <= inflation <= 3.839)
    year_macro_good = gdp_good and inflation_good
    
    # New: check monthly unemployment for each month
    for t in range(year_start_month, year_end_month):
        unemployed = 0
        employed = 0
        ...
        # Monthly unemployment constraint
        unemployment_good = (0.035 <= monthly_unemployment <= 0.148)
        
        if year_macro_good and unemployment_good:
            macro_good_months.append(t)

print(f"Found {len(macro_good_months)} months with good macro performance")

# ========== 4. Extract micro-good decisions from macro-good months ==========
print("\nStep 2: Extracting micro-good decisions from macro-good months...")

good_decisions = []

# Precompute each agent's yearly MPC
agent_year_mpc = {}
...
print(f"Extracted {len(good_decisions)} good decisions")
if len(good_decisions) > 0:
    print(f"Percentage of total decisions: {len(good_decisions)/(max_t*100)*100:.2f}%")

# ========== 5. Save ==========
if len(good_decisions) == 0:
    print("\nWarning: No qualified decisions found!")
    print("Suggestions:")
    print("1. Check if macro constraints are too strict")
    print("2. Relax yearly MPC range (e.g., [-2, 3])")
    print("3. Check if data is loaded correctly")
else:
    df_good = pd.DataFrame(good_decisions)

    # Perform employment/unemployment balancing
    ...
    print(f"Original: employed {len(employed_decisions)}, unemployed {len(unemployed_decisions)}")
    ...
    if len(unemployed_decisions) > 0 and len(employed_decisions) > 0:
        ...
        if original_ratio >= target_unemployed_ratio:
            print(f"Unemployment ratio already meets target ({original_ratio*100:.1f}% >= {target_unemployed_ratio*100:.1f}%)")
        else:
        ...
        print("Cannot balance: missing employment or unemployment records")

    print("\nData quality check:")
    print(f"Years included: {sorted(df_good['year'].unique())}")
    print(f"Total decisions: {len(df_good)}")
    print(f"Employment ratio: {(df_good['work_decision']==1).sum()/len(df_good)*100:.1f}%")
    print(f"Unemployment ratio: {(df_good['work_decision']==0).sum()/len(df_good)*100:.1f}%")
    print(f"Average score: {df_good['score'].mean():.2f}")
    
    mpc_valid = df_good[df_good['yearly_mpc'] != 0.0]['yearly_mpc']
    if len(mpc_valid) > 0:
        print("\nYearly MPC statistics:")
        print(f"  Count: {len(mpc_valid)}")
        print(f"  Mean: {mpc_valid.mean():.2f}")
        print(f"  Median: {mpc_valid.median():.2f}")
        print(f"  Range: [{mpc_valid.min():.2f}, {mpc_valid.max():.2f}]")
        print(f"  In [0.05, 0.9]: {((mpc_valid>=0.05)&(mpc_valid<=0.9)).sum()/len(mpc_valid)*100:.1f}%")
    
    output_path = '/workspace/QWEN2.5_RL1/data/gpt-3-noperception-reflection-1-100agents-240months/good_decisions.csv'
